    ...
    public static class Utilitats
    {
        ...
        **/
        ...
        public static void exportGridToPDF(DataGridView dgw, string filename)
        {
            ...
            var saveFiledDialoge = new SaveFileDialog();
            saveFiledDialoge.FileName = filename;
            saveFiledDialoge.DefaultExt = ".pdf";
            if (saveFiledDialoge.ShowDialog() == DialogResult.OK)
            {
                using (FileStream stream = new FileStream(saveFiledDialoge.FileName, FileMode.Create))
                {
